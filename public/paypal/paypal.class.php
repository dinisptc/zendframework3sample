				...
				if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])){
					...
					*/
					...
					if('Completed' == $httpParsedResponseAr["PAYMENTINFO_0_PAYMENTSTATUS"]){
                                                ...
                                                //{ 
                                                    //$userID = $_SESSION['identidade']['userId'];  
                                                    
                                                    $expiredate=$this->add_date(date("Y-m-d H:i:s"), 12);
				                    //$datau['usr_premium_expire'] = $expiredate;
                                                
                                                    $qb = $this->getEntityManager()->createQueryBuilder();  
                                                    $q = $qb->update('AuthDoctrine\Entity\User', 'u')  
                                                        ->set('u.usrl_id', '?1')    
                                                        ->set('u.usr_premium_expire', '?2')
                                                        ->where('u.usrId = ?3')  
                                                        ->setParameter(1, 4) //conta premium
                                                        ->setParameter(2, $expiredate)
                                                        ->setParameter(2, $item['ItemNumber'])   
                                                        ->getQuery();  
                                                    $p = $q->execute();
                                                    
                                                    echo '<div style="color:green">Payment Received! Your will get your Premium Account in seconds!</div>';
