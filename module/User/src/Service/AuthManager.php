            ...
            foreach ($items as $item) {
                ...
                if (is_array($actionList) && in_array($actionName, $actionList) ||
                    ...
                    if ($allow=='*')
                        ...
                        {
                            return true; // Only authenticated user and Admin is allowed to see the page.
                    ...
                    } else if($allow=='m' && $this->authService->hasIdentity()) {
                        
                        $user = $this->entityManager->getRepository(User::class)
                        ->findOneByEmail($this->authService->getIdentity());
                        if(($user->getPerfil()==User::PERFIL_MEMBER) || ($user->getPerfil()==User::PERFIL_ADMIN) || ($user->getPerfil()==User::PERFIL_PRO))
                        {
                            return true; // Only authenticated user and Admin is allowed to see the page.
                    ...
                    } else if($allow=='p' && $this->authService->hasIdentity()) {
                        
                        
                          $user = $this->entityManager->getRepository(User::class)
                        ->findOneByEmail($this->authService->getIdentity());
                        if(($user->getPerfil()==User::PERFIL_PRO) || ($user->getPerfil()==User::PERFIL_ADMIN))
                        {
                            return true; // Only authenticated user and Admin is allowed to see the page.
