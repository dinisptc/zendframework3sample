 ...
 * with such an email.
 */
class EmpresaExistsValidator extends AbstractValidator 
{
    /**
    ...
    protected $messageTemplates = array(
        ...
        self::USER_EXISTS  => "Another company with such an email already exists"        
    );
    ...
    public function isValid($value) 
    {
        ...
        $empresa = $entityManager->getRepository(Empresas::class)
                ->findOneByEmail($value);
        
        if($this->options['empresa']==null) {
            $isValid = ($empresa==null);
        } else {
            if($this->options['empresa']->getEmail()!=$value && $empresa!=null) 
                $isValid = false;
            else 
                $isValid = true;
        }
